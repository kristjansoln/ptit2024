...
UDP_IP_CMD = "127.0.0.1"
UDP_IP_TTS = "127.0.0.1"
UDP_PORT_CMD = 15005
UDP_PORT_TTS = 15006
...
def reset_game():
    ...
    # Send out a message TTS
    MESSAGE = "Bloody hell!"
    sock.sendto(bytes(MESSAGE, "utf8"), (UDP_IP_TTS, UDP_PORT_TTS))
...
# Define socket for receiving commands
sock = socket.socket(
    socket.AF_INET,
    socket.SOCK_DGRAM,
)  # UDP
sock.bind((UDP_IP_CMD, UDP_PORT_CMD))
sock.setblocking(0)

# Define socket for sending TTS commands
sock_tts = socket.socket(
    socket.AF_INET,
    socket.SOCK_DGRAM,
)  # UDP
